...
\author{Jaime G\'omez-Ram\'irez        \and
        Jose Luis P\'erez-Vel\'azquez %etc.
}
...
\begin{figure}
\centering
\subfloat[Sphere]{\includegraphics[width = 2.5cm, height=2.5cm,keepaspectratio]{../figures/fig1-a-sphere}} 
\subfloat[Torus]{\includegraphics[width = 2.5cm, height=2.5cm,keepaspectratio]{../figures/fig1-b-torus}}
\subfloat[Double torus]{\includegraphics[width = 4cm, height=4cm,keepaspectratio]{../figures/fig1c-double_torus}}
\subfloat[Balloon]{\includegraphics[width = 3cm, height=3cm,keepaspectratio]{../figures/fig1d-balloon}} 
...
%Since the goal of algebraic topology is to classify topological spaces. . 
...
%https://people.maths.ox.ac.uk/porterm/research/Dissertation-stolz2014-Corr.pdf
\begin{figure}[h]
...
% n different graphs provide n diffrent scs
        \centering
        \includegraphics[width=0.8\linewidth]{../figures/fig2-sc}
...
\label{fig:vrcomplex}
\end{figure}
%The figure shows the simplicial complex obtained from different graphs and a fixed threshold, $\epsilon$ such that two vertices, $u, v$, are connected if the distance between u and v is less than the threshold, $d(u,v) < \epsilon$.
% Persistent Homology
...
%This graph filtration approach along with the persistent homology framework can be used to differentiate between classes of networks, so it can be a sort of hierarchical classifier. For example, used in differentiating between glucose metabolic network using 102 ROI from FDG-PET from ADHD, ASD and pediatric controls. Here biblio on filtration applied to biology
...
%https://www.youtube.com/watch?v=XOZN3XZdoO0 (Chad Giusti)
        \centering
        \includegraphics[width=0.8\linewidth]{../figures/filtration}
...
%a homology theory adapted to a computational context, for instance, in analysis of large data sets. It keeps track of homology classes which stay ‘persistent’ when the approximate image of a space gets refined to higher resolutions.
...
\begin{figure}[h]
        \centering
        \includegraphics[width=0.8\linewidth]{../figures/holesindata}
...
%Simplicial complexes is a generalization of graphs that overcomes this limitation.
...
\begin{figure}[h]
  %Figure 4 pacemaker coactivity (model generated not fMRI)
        \centering
        \includegraphics[width=0.5\linewidth]{../figures/figface}
...
The idea behind persistent homology is to identify those features in the dataset that persist across multiples scales as revealed in a filtration process. A graph filtration is in essence the outcome of the continuous variation of a threshold $\epsilon$. Thus, for a given threshold $\epsilon$ two nodes a and b are linked if $d(a, b) \leq \epsilon$, when $\epsilon = 0$ the network is totally disconnected and has N (the number of nodes) components. The number of connected components increases as $\epsilon$ increases, at some point components start forming larger aggregates reducing the number of components to one in which case the network is fully connected. 
...
For p regions of interest there are $2^p$ subsets candidates to form a topology, the power set is a representation of the underlying topology (see Appendix for definition of topology).
%Given a (measurement) set $X$ and a rule of connection, the topological space is a simplicial complex and its elements are simplices.
...
While the binary networks have at most 1-semplices, the Rips complex has up to p-1-simplices, so the Rips complex can have faces as well (Figure \cite{ex:dyadic}). 
...
The measurement set $X$ and the distance $C_X$ form the metric space $(X,C_X)$. The network constructed by thresholding the correlation matrix between nodes is trivially constructed as follows: if $C(X_i,X_j) \leq \epsilon$ there is a link between nodes $X_i$ and $X_j$, otherwise the nodes are not connected. We focus on the Default mode Network , a network comprising 4 nodes in mPFC, PCC and leaft and right MTP.
...
\begin{figure}[h]
%Figure adj amtrix and network a -b
        \centering
